...
class SerializersTest {	
	...
	val presentationTwo = Presentation(Some(2L), Some("ccc"), Some("15-16"), Some("Presenter 1, Presenter 2"), Some("desc2"), Some(2L))
	val presentationThree = Presentation(Some(3L), Some("ddd"), Some("16-17"), Some("Presenter 1, Presenter 3"), Some("desc3"), Some(2L))
	...
	val eventListJson = JsArray(Seq(
									JsObject(Seq(
												...
												"presentations" -> JsArray(
												    Seq(JsObject(Seq("id" -> JsNumber(2L), "name" -> JsString("ccc"), "time" -> JsString("15-16"), "presenters" -> JsString("Presenter 1, Presenter 2"), "description" -> JsString("desc2"), "eventId" -> JsNumber(2L))),
												        JsObject(Seq("id" -> JsNumber(3L), "name" -> JsString("ddd"), "time" -> JsString("16-17"), "presenters" -> JsString("Presenter 1, Presenter 3"), "description" -> JsString("desc3"), "eventId" -> JsNumber(2L)))))
											)),
									JsObject(Seq(
